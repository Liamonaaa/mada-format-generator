...
- Vehicle abandonment format supports browser image paste, upload, drag-and-drop, preview, remove actions, and Discord-oriented rich clipboard copy
...
5. In the vehicle abandonment format, you can paste an image from the clipboard or upload one directly in the browser, use `העתק` for plain text, or use `העתק לדיסקורד` for a best-effort clipboard copy that includes both the text and the attached image.
